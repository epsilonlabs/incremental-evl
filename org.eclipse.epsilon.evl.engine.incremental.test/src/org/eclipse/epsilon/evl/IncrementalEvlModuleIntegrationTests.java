...
public class IncrementalEvlModuleIntegrationTests {
	...
	private class InMemoryEvlTraceManager extends AbstractEvlExecutionTraceManager  {

		@Override
		public boolean persistTraceInformation() {
			// TODO Implement IEolExecutionTraceManager<IEvlModuleExecution>.persistTraceInformation
			throw new UnsupportedOperationException("Unimplemented Method    IEolExecutionTraceManager<IEvlModuleExecution>.persistTraceInformation invoked.");
		}
	...
	public void setup() throws URISyntaxException {
		...
		evlFile = new File(IncrementalEvlModuleIntegrationTests.class.getResource("test.evl").toURI());
	}
	...
	/** Check that the ExecutionTrace model is being populated */
	@Test
	public void testCreateCst() throws URISyntaxException, Exception {
		...
		assertThat(module.evlExecution.moduleElements().get().size(), is(1));
		IContextTrace cContext = (IContextTrace) module.evlExecution.moduleElements().get().peek();
		assertThat(cContext.constraints().get().size(), is(5));
		assertThat(cContext.guard().get(), is(notNullValue()));
		
		// UnGuardedSimple
		IInvariantTrace unGuardedSimpleTrace = cContext.constraints().get().poll();
		assertThat(unGuardedSimpleTrace.getName(), is("UnGuardedSimple"));
		assertThat(unGuardedSimpleTrace.guard().get(), is(nullValue()));
		assertThat(unGuardedSimpleTrace.check().get(), is(notNullValue()));
		assertThat(unGuardedSimpleTrace.message().get(), is(nullValue()));
		cContext.constraints().get().add(unGuardedSimpleTrace);
		
		// GuardedSimple
		IInvariantTrace guardedSimpleTrace = cContext.constraints().get().poll();
		assertThat(guardedSimpleTrace.getName(), is("GuardedSimple"));
		assertThat(guardedSimpleTrace.guard().get(), is(notNullValue()));
		assertThat(guardedSimpleTrace.check().get(), is(notNullValue()));
		assertThat(guardedSimpleTrace.message().get(), is(nullValue()));
		cContext.constraints().get().add(guardedSimpleTrace);
		
		// GuardedWithMessage
		IInvariantTrace guardedWithMessageTrace = cContext.constraints().get().poll();
		assertThat(guardedWithMessageTrace.getName(), is("GuardedWithMessage"));
		assertThat(guardedWithMessageTrace.guard().get(), is(notNullValue()));
		assertThat(guardedWithMessageTrace.check().get(), is(notNullValue()));
		assertThat(guardedWithMessageTrace.message().get(), is(notNullValue()));
		cContext.constraints().get().add(guardedWithMessageTrace);
		
		// UnguardedWithFix
		IInvariantTrace unguardedWithFixTrace = cContext.constraints().get().poll();
		assertThat(unguardedWithFixTrace.getName(), is("UnguardedWithFix"));
		assertThat(unguardedWithFixTrace.guard().get(), is(nullValue()));
		assertThat(unguardedWithFixTrace.check().get(), is(notNullValue()));
		assertThat(unguardedWithFixTrace.message().get(), is(nullValue()));
		cContext.constraints().get().add(unguardedWithFixTrace);
