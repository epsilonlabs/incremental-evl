...
public class ElementAccessHasElementGremlin extends Feature
     ...
     */
    public ElementAccessHasElementGremlin (IElementAccess source, GraphTraversalSource gts) {
        super(true);
        this.source = source;
        this.gts = gts;
    }
    ...
    public boolean conflict(IModelElementTrace target) {
        ...
        GraphTraversalSource g = startTraversal();
        try {
            result |= delegate == null ? g.V(source.getId()).out("element").hasNext() : g.E(delegate).outV().hasId(target.getId()).hasNext();
        }
        finally {
            finishTraversal(g);
        }
