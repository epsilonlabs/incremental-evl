...
public class ModelElementTraceHasModelTraceGremlin extends Feature
        implements IModelElementTraceHasModelTrace, TinkerpopDelegate<Edge> {
     ...
     */
    public ModelElementTraceHasModelTraceGremlin (
   ...
   /**
        ...
        try (ActiveTraversal agts = new ActiveTraversal(gts)) {
            ...
            try (ActiveTraversal agts2 = new ActiveTraversal(gts)) {
                result |= delegate == null ? false : (target.elements().isUnique() &&
                        agts2.V(target.getId()).out("elements").hasId(source.getId()).hasNext());
            }
            catch (Exception ex) {
                result = false;
            }
    ...
    public boolean related(IModelTrace target) {
        ...
        try (ActiveTraversal agts = new ActiveTraversal(gts)) {
            inTarget = agts.V(target.getId()).out("elements").hasId(source.getId()).hasNext();
        } catch (Exception e) {
            throw new IllegalStateException("There was an error during graph traversal.", e);
        }
        try (ActiveTraversal agts = new ActiveTraversal(gts)) {
