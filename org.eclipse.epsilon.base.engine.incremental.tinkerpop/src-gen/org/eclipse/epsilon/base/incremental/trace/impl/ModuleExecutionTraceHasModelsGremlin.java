...
public class ModuleExecutionTraceHasModelsGremlin extends Feature
     ...
     */
    public ModuleExecutionTraceHasModelsGremlin (IModuleExecutionTrace source) {
        super(true);
        this.source = source;
    }
    ...
    public Iterator<IModelAccess> get() {
        return new GremlinUtils.IncrementalFactoryIterator<IModelAccess, Vertex>(getRaw(), graph);
    }
    ...
    public boolean related(IModelAccess target) {
		...
		return g.V(source.getId()).out("models").hasId(target.getId()).hasNext();
	}
