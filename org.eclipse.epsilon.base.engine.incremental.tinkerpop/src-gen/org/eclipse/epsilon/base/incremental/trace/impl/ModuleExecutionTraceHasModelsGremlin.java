...
public class ModuleExecutionTraceHasModelsGremlin extends Feature
        implements IModuleExecutionTraceHasModels, TinkerpopDelegate<Edge> {
     ...
     */
    public ModuleExecutionTraceHasModelsGremlin (
    ...
    public Iterator<IModelAccess> get() {
        return new GremlinUtils.IncrementalFactoryIterator<IModelAccess>(getRaw(),
                gts, factory);
    }
    ...
    public GraphTraversal<Vertex, Vertex> getRaw() {
        ...
        try (ActiveTraversal agts = new ActiveTraversal(gts)) {
            result = agts.V(source.getId()).out("models");
        } catch (Exception e) {
    ...
    public boolean conflict(IModelAccess target) {
        ...
        try (ActiveTraversal agts = new ActiveTraversal(gts)) {
	        if (isUnique()) {
	            GraphTraversal<Vertex, Vertex> gt =  agts.V(source.getId()).out("models");
                for (Entry<String, Object> id : target.getIdProperties().entrySet()) {
                    gt.has(id.getKey(), id.getValue());
                }
                result |= gt.hasNext();
                if (target.modelTrace().get() != null) {
	                gt.as("s")
	                    .out("modelTrace").hasId(target.modelTrace().get().getId())
	                    .select("s");
	               result |= gt.hasNext();
	            }
	            else {
    ...
    public boolean related(IModelAccess target) {
        ...
        boolean result = false;
        try (ActiveTraversal agts = new ActiveTraversal(gts)) {
		  result = agts.V(source.getId()).out("models").hasId(target.getId()).hasNext();
		} catch (Exception e) {
            throw new IllegalStateException("There was an error during graph traversal.", e);
        }
        return result;
    ...
    public void set(IModelAccess target) {
        try (ActiveTraversal agts = new ActiveTraversal(gts)) {
            agts.V(source.getId()).addE("models")
