...
public interface ModelHasTypes {
     ...
     * The relation is created if there are no conflicts (see {@link ExecutionHasAccesses#conflict(ModelType)}).
     * If the reference has an opposite, that relation is also craeted.
     *
     * @see ExecutionHasAccesses#conflict(ModelType)
     * @see ExecutionHasAccesses#related(ModelType)
     */
    boolean create(ModelType target);
     ...
     * @see ExecutionHasAccesses#related(ModelType)
     */    
    boolean destroy(ModelType target);
     ...
     *  </li>
     */
    boolean conflict(ModelType target);
    
    /**
     * Returns true if the target is already related via this reference.
     */
    boolean related(ModelType target);
     ...
     * relation.
     *
     * @see ExecutionHasAccesses#create(ModelType)
     */
    void set(ModelType target);

    /**
     * Remove a value for the reference. This method should be only accessed by classes in the
     * relation.
     *
     * @see ExecutionHasAccesses#destroy(ModelType)
     */
    void remove(ModelType target);
}