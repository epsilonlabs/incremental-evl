...
public class InvariantTraceHasMessageGremlin extends Feature
        implements IInvariantTraceHasMessage, TinkerpopDelegate<Edge> {
   ...
   /**
    ...
    public IMessageTrace get() {
        if (delegate == null) {
            try (ActiveTraversal agts = new ActiveTraversal(gts)) {
                GraphTraversal<Vertex, Edge> et = agts.V(source.getId()).outE("message");
                if (et.hasNext()) {
                    delegate = et.next();
                }
            } catch (Exception e) {
    ...
    public boolean conflict(IMessageTrace target) {
        ...
        try (ActiveTraversal agts = new ActiveTraversal(gts)) {
            result |= delegate == null ?
                    agts.V(source.getId()).out("message").hasNext() :
    ...
    public boolean related(IMessageTrace target) {
        ...
        boolean result = false;
        try (ActiveTraversal agts = new ActiveTraversal(gts)) {
		  result = agts.E(delegate).inV().hasId(target.getId()).hasNext() && source.equals(target.invariant().get());
		} catch (Exception e) {
            throw new IllegalStateException("There was an error during graph traversal.", e);
        }
        return result;
    ...
    public void set(IMessageTrace target) {
        try (ActiveTraversal agts = new ActiveTraversal(gts)) {
            delegate = agts.V(source.getId()).addE("message")
