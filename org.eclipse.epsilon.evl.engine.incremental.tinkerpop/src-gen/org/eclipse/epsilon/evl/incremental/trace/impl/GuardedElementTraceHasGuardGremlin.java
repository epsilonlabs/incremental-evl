...
public class GuardedElementTraceHasGuardGremlin extends Feature
    ...
    public boolean conflict(IGuardTrace target) {
        ...
        GraphTraversalSource g = startTraversal();
        try {
            result |= delegate == null ? g.V(source.getId()).out("guard").hasNext() : g.E(delegate).inV().hasId(target.getId()).hasNext();
    ...
    public boolean related(IGuardTrace target) {
        ...
        GraphTraversalSource g = startTraversal();
        try {
		  result = g.E(delegate).inV().hasId(target.getId()).hasNext() && source.equals(target.limits().get());
		}
		finally {
            finishTraversal(g);
        }
