...
public interface GuardedElementHasGuard {
     ...
     * The relation is created if there are no conflicts (see {@link ExecutionHasAccesses#conflict(Guard)}).
     * If the reference has an opposite, that relation is also craeted.
     *
     * @see ExecutionHasAccesses#conflict(Guard)
     * @see ExecutionHasAccesses#related(Guard)
     */
    boolean create(Guard target);
     ...
     * relation.
     *
     * @see ExecutionHasAccesses#create(Guard)
     */
    void set(Guard target);

    /**
     * Remove a value for the reference. This method should be only accessed by classes in the
     * relation.
     *
     * @see ExecutionHasAccesses#destroy(Guard)
     */
    void remove(Guard target);
}