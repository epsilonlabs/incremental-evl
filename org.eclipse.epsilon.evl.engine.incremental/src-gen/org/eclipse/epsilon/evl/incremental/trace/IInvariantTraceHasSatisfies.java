...
public interface IInvariantTraceHasSatisfies {
     ...
     * The relation is created if there are no conflicts (see {@link ExecutionHasAccesses#conflict(ISatisfiesTrace)}).
     * If the reference has an opposite, that relation is also created.
     *
     * @see ExecutionHasAccesses#conflict(ISatisfiesTrace)
     * @see ExecutionHasAccesses#related(ISatisfiesTrace)
     * @param target The ISatisfiesTrace to create a relation with
     * @throws TraceModelConflictRelation if a relation to another ISatisfiesTrace exists
     */
    boolean create(ISatisfiesTrace target) throws TraceModelConflictRelation;
     ...
     * @see ExecutionHasAccesses#related(ISatisfiesTrace)
     */    
    boolean destroy(ISatisfiesTrace target);
     ...
     *  </li>
     */
    boolean conflict(ISatisfiesTrace target);
    
    /**
     * Returns true if the target is already related via this reference.
     */
    boolean related(ISatisfiesTrace target);
     ...
     * relation.
     *
     * @see ExecutionHasAccesses#create(ISatisfiesTrace)
     */
    void set(ISatisfiesTrace target);

    /**
     * Remove a value for the reference. This method should be only accessed by classes in the
     * relation.
     *
     * @see ExecutionHasAccesses#destroy(ISatisfiesTrace)
     */
    void remove(ISatisfiesTrace target);
}