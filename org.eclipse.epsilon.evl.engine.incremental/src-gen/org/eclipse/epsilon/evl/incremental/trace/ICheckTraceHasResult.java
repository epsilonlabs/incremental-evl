...
public interface ICheckTraceHasResult {
     ...
     * The relation is created if there are no conflicts (see {@link ExecutionHasAccesses#conflict(ICheckResult)}).
     * If the reference has an opposite, that relation is also created.
     *
     * @see ExecutionHasAccesses#conflict(ICheckResult)
     * @see ExecutionHasAccesses#related(ICheckResult)
     * @param target The ICheckResult to create a relation with
     * @throws TraceModelConflictRelation if a relation to another ICheckResult exists
     */
    boolean create(ICheckResult target) throws TraceModelConflictRelation;
     ...
     * @see ExecutionHasAccesses#related(ICheckResult)
     */    
    boolean destroy(ICheckResult target);
     ...
     *  </li>
     */
    boolean conflict(ICheckResult target);
    
    /**
     * Returns true if the target is already related via this reference.
     */
    boolean related(ICheckResult target);
     ...
     * relation.
     *
     * @see ExecutionHasAccesses#create(ICheckResult)
     */
    void set(ICheckResult target);

    /**
     * Remove a value for the reference. This method should be only accessed by classes in the
     * relation.
     *
     * @see ExecutionHasAccesses#destroy(ICheckResult)
     */
    void remove(ICheckResult target);
}