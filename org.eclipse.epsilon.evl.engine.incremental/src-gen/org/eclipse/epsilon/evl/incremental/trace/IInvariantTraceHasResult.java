...
public interface IInvariantTraceHasResult {
     ...
     * The relation is created if there are no conflicts (see {@link ExecutionHasAccesses#conflict(IGuardResult)}).
     * If the reference has an opposite, that relation is also created.
     *
     * @see ExecutionHasAccesses#conflict(IGuardResult)
     * @see ExecutionHasAccesses#related(IGuardResult)
     * @param target The IInvariantResult to create a relation with
     * @throws TraceModelConflictRelation if a relation to another IInvariantResult exists
     */
    boolean create(IGuardResult target) throws TraceModelConflictRelation;
     ...
     * @see ExecutionHasAccesses#related(IGuardResult)
     */    
    boolean destroy(IGuardResult target);
     ...
     *  </li>
     */
    boolean conflict(IGuardResult target);
    
    /**
     * Returns true if the target is already related via this reference.
     */
    boolean related(IGuardResult target);
     ...
     * relation.
     *
     * @see ExecutionHasAccesses#create(IGuardResult)
     */
    void set(IGuardResult target);

    /**
     * Remove a value for the reference. This method should be only accessed by classes in the
     * relation.
     *
     * @see ExecutionHasAccesses#destroy(IGuardResult)
     */
    void remove(IGuardResult target);
}