...
public interface IGuardedElementTraceHasGuard {
     ...
     * The relation is created if there are no conflicts (see {@link ExecutionHasAccesses#conflict(IGuardTrace)}).
     * If the reference has an opposite, that relation is also created.
     *
     * @see ExecutionHasAccesses#conflict(IGuardTrace)
     * @see ExecutionHasAccesses#related(IGuardTrace)
     * @param target The IGuardTrace to create a relation with
     * @throws TraceModelConflictRelation if a relation to another IGuardTrace exists
     */
    boolean create(IGuardTrace target) throws TraceModelConflictRelation;
     ...
     * @see ExecutionHasAccesses#related(IGuardTrace)
     */    
    boolean destroy(IGuardTrace target);
     ...
     *  </li>
     */
    boolean conflict(IGuardTrace target);
    
    /**
     * Returns true if the target is already related via this reference.
     */
    boolean related(IGuardTrace target);
     ...
     * relation.
     *
     * @see ExecutionHasAccesses#create(IGuardTrace)
     */
    void set(IGuardTrace target);

    /**
     * Remove a value for the reference. This method should be only accessed by classes in the
     * relation.
     *
     * @see ExecutionHasAccesses#destroy(IGuardTrace)
     */
    void remove(IGuardTrace target);
}